...
function compare_outputs {
  ...
  PROJECT_ANSWER_PATH5=${GRADER_ROOT}/temp/log_output/extensions.txt
  PROJECT_ANSWER_PATH6=${GRADER_ROOT}/temp/log_output/http_errors.txt
  ...
  TEST_ANSWER_PATH5=${GRADER_ROOT}/tests/${test_folder}/log_output/extensions.txt
  TEST_ANSWER_PATH6=${GRADER_ROOT}/tests/${test_folder}/log_output/http_errors.txt
  ...
  DIFF_RESULT5=$(diff -bB ${PROJECT_ANSWER_PATH5} ${TEST_ANSWER_PATH5} | wc -l)
  if [ "${DIFF_RESULT5}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH5} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (extensions.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (extensions.txt)"
    diff ${PROJECT_ANSWER_PATH5} ${TEST_ANSWER_PATH5}
  fi
  
  DIFF_RESULT6=$(diff -bB ${PROJECT_ANSWER_PATH6} ${TEST_ANSWER_PATH6} | wc -l)
  if [ "${DIFF_RESULT6}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH6} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (http_errors.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (http_errors.txt)"
    diff ${PROJECT_ANSWER_PATH6} ${TEST_ANSWER_PATH6}
  fi
}

function run_all_tests {
  TEST_FOLDERS=$(ls ${GRADER_ROOT}/tests)
  NUM_TESTS=$(($(echo $(echo ${TEST_FOLDERS} | wc -w)) * 6))
  PASS_CNT=0
